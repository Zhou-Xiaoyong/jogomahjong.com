/*
 * Mahjong Relaxing — zen mode of Solitaire.
 * No time pressure, unlimited hints and automatic reshuffling
 * when there are no more moves. Traditional Chinese tiles with
...
(function () {
  ...
  // Flatter, gentler layout: two layers, no high apex.
  const layout = [
    ...rect(1, 0, 10, 1, 0),
    ...rect(0, 1, 12, 5, 0),
    ...rect(1, 6, 10, 1, 0),
  ...
  function startGame() {
    ...
    render();
    updateStats();
    setMessage("Relaxing mode: no rush, no fatal mistakes. Use as many hints as you want and take a deep breath.");
  ...
  function selectTile(uid) {
    ...
    if (!selected) {
      ...
      setMessage("No rush. Find the matching tile whenever you want.");
      return;
    }
    ...
    if (matches(selected.type, tile.type)) {
      ...
      setTimeout(() => {
        ...
        if (tiles.every(item => item.removed)) {
          ...
          setMessage("Done! Board cleared at your own pace. How about a relaxing Pyramid game now?");
          clearInterval(timer);
        } else {
          const pair = findHintPair();
          if (!pair) {
            setMessage("No free pairs. I'll shuffle so you can keep relaxing...");
            setTimeout(autoShuffle, 1200);
          } else {
            setMessage("Pair removed. Continue at your own pace.");
          }
        }
      }, 320);
    } else {
      ...
      markSelected();
      updateStats();
      setMessage("They don't match, but no problem — in relaxing mode there's no penalty.");
    }
  }
  ...
  function showHint() {
    ...
    const pair = findHintPair();
    if (!pair) {
      setMessage("No free pairs right now. Shuffling automatically...");
    ...
    setMessage("Free hint: match the two highlighted tiles. No usage limit.");
  }
  ...
  function shuffleRemaining() {
    ...
    render();
    updateStats();
    setMessage("Tiles shuffled. Breathe and continue.");
  }
