/*
 * Mahjong Chain — Connect with combo (chain) system.
 * Connect pairs in rapid sequence to multiply points.
...
(function () {
  ...
  function startGame() {
    ...
    renderGrid();
    updateStats();
    setMessage("Connect pairs in rapid sequence to build a chain. The longer the chain, the more points!");
  ...
  function selectTile(r, c) {
    ...
    if (selected.type.id !== cell.type.id) {
      ...
      setMessage("Different tiles — chain broken! Try another.");
      return;
    }
    ...
    if (!path) {
      ...
      const el2 = getTileEl(r, c);
      if (el2) el2.classList.add("selected");
      setMessage("Cannot connect with at most 2 corners. Chain broken!");
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (removedPairs >= totalPairs) {
        ...
        setMessage(`Board cleared! Final score: ${score} (max chain: ${bestCombo}).`);
        clearInterval(timer);
        clearTimeout(comboTimer);
      } else {
        const pair = findHintPair();
        if (!pair) {
          setMessage("No connectable pairs. Shuffling...");
  ...
  function shuffleRemaining() {
    ...
    if (!pair) {
      ...
      setMessage("Tiles shuffled! Continue the chain.");
    }
  }
  ...
  function registerMatch() {
    ...
    // Points: base 10 multiplied by the chain length.
    const gained = 10 * combo;
    score += gained;
    const msg = combo > 1
      ? `Chain x${combo}! +${gained} points. Keep going fast to increase!`
      : `Pair connected! +${gained} points.`;
    setMessage(msg);
    ...
    comboTimer = setTimeout(() => {
      if (combo > 0) {
        combo = 0;
        updateStats();
        setMessage("Chain expired. Connect a pair to start a new one.");
      }
    }, COMBO_WINDOW);
  }
  ...
  function showHint() {
    ...
    const pair = findHintPair();
    if (!pair) {
      setMessage("No connectable pairs. Restarting...");
      return;
    }
    ...
    setMessage("Hint: connect these two tiles. (Chain has been reset.)");
  }
