/*
 * Mahjong Pyramid — solitaire with a stepped pyramid layout.
 * Four centered layers form a classic pyramid (ziggurat).
 * Traditional Chinese tiles with annotations in English.
...
(function () {
  ...
  // Stepped pyramid layout (ziggurat):
  //   z=0  ->  8 cols x 4 rows  (wide base)
  //   z=1  ->  6 cols x 3 rows  (middle layer)
  //   z=2  ->  4 cols x 2 rows  (upper layer)
  //   z=3  ->  2 cols x 1 row   (top)
  // Total = 32 + 18 + 8 + 2 = 60 tiles (30 pairs).
  // Each layer is centered horizontally relative to the base.
  const BASE_COLS = 8;
  const layout = [
    ...rect(0, 0, 8, 4, 0),
    ...rect(1, 0, 6, 3, 1),
  ...
  // Same rules as classic solitaire: flowers match each other,
  // seasons match each other, other tiles require equal id.
  ...
  function buildDeck(count) {
    ...
    const deck = [];
    // For the pyramid we use only the core set (no flowers/seasons),
    // keeping symmetry and making it easier to solve.
  ...
  function startGame() {
    ...
    render();
    updateStats();
    setMessage("Pyramid structure! Top tiles block the ones below. Start from the free edges.");
    timer = setInterval(updateTime, 1000);
  }

  function render() {
    board.innerHTML = "";
    // Horizontally centers the pyramid within the board (720px).
  ...
  function selectTile(uid) {
    ...
    if (!selected) {
      ...
      setMessage("Tile selected. Find a matching tile that is also free.");
      return;
    }
    ...
    if (selected.uid === tile.uid) {
      selected = null;
      markSelected();
      setMessage("Selection cleared. Keep exploring the pyramid.");
      return;
    }
    ...
    if (matches(selected.type, tile.type)) {
      ...
      setTimeout(() => {
        ...
        if (tiles.every(item => item.removed)) {
          ...
          setMessage("Pyramid dismantled! You won. Try to beat your time next.");
          clearInterval(timer);
        } else {
          const pair = findHintPair();
          setMessage(pair ? "Pair removed! Keep climbing the pyramid." : "No free pairs. Use shuffle to unlock.");
        }
      }, 320);
    } else {
  ...
  function showHint() {
    ...
    const pair = findHintPair();
    if (!pair) {
      setMessage("No free pairs at the moment. Shuffle to continue.");
      return;
    }
    ...
    setMessage("Hint: match the two highlighted tiles.");
  }
  ...
  function shuffleRemaining() {
    ...
    render();
    updateStats();
    setMessage("Remaining tiles shuffled. Continue!");
  }
