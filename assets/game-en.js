/*
 * Mahjong Solitaire — classic HTML5 version.
 * Uses traditional Chinese tiles (Unicode) with annotations in English.
...
(function () {
  ...
  const offsetY = 30;
  const layerShift = 8;
  // Vertical overlap factor that creates the "tilted" tile effect
  // characteristic of traditional Mahjong Solitaire.
  const rowFactor = 0.46;

  const T = window.MAHJONG_TILES;
  // Main set: winds, dragons, characters, bamboos and circles.
  const tileTypes = T.core;
  // Flowers and seasons match within their own group (traditional rule).
  const flowerTypes = T.flowers;
  const seasonTypes = T.seasons;

  const layout = [
    // Base layer: shape inspired by the classic "turtle"
    ...rect(4, 0, 4, 1, 0),
    ...rect(2, 1, 8, 1, 0),
    ...rect(0, 2, 12, 4, 0),
    ...rect(2, 6, 8, 1, 0),
  ...
  // Two tiles match if they have the same id, or if both are flowers, or if both
  // are seasons (classic Chinese rule for special groups).
  ...
  function buildDeck(count) {
    ...
    const deck = [];
    // Each type appears in sufficient quantity to form pairs.
  ...
  function startGame() {
    ...
    render();
    updateStats();
    setMessage("Match two identical free tiles to remove them. Flowers match with each other, seasons match with each other. Good luck!");
  ...
  function selectTile(uid) {
    ...
    if (!selected) {
      ...
      setMessage("Now find the matching tile that is also free.");
      return;
    }
    ...
    if (selected.uid === tile.uid) {
      selected = null;
      markSelected();
      setMessage("Selection cleared. Take your time.");
      return;
    }
    ...
    if (matches(selected.type, tile.type)) {
      ...
      setTimeout(() => {
        ...
        if (tiles.every(item => item.removed)) {
          ...
          setMessage("Congratulations! You cleared the board! How about trying to win in less time?");
          clearInterval(timer);
        } else {
          const pair = findHintPair();
          setMessage(pair ? "Pair removed! Keep finding matches." : "No free pairs right now. Use shuffle to continue.");
        }
      }, 320);
    } else {
  ...
  function showHint() {
    ...
    setMessage("Hint: try matching the two highlighted tiles.");
  }
  ...
  function shuffleRemaining() {
    ...
    render();
    updateStats();
    setMessage("Remaining tiles shuffled. Good luck!");
  }
