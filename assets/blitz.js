/*
 * Mahjong Blitz — modo corrida de 60 segundos.
 * O tabuleiro se renova automaticamente para manter o ritmo.
...
(function () {
  ...
  // Layout simples: 3 camadas, base 8x5
  const layerConfigs = [
  ...
  function startGame() {
    ...
    renderBoard();
    updateStats();
    setMessage("Vai! 60 segundos — faça o máximo de pares!");
  }
  ...
  function checkRefill() {
    ...
    // Se houver poucas peças ou nenhum par disponível, reabastecer
  ...
  function refillBoard() {
    ...
    // Manter as peças restantes e adicionar novas
    const remaining = tiles.filter(t => !t.removed);
    const layout = buildLayout();
    ...
    renderBoard();
    updateStats();
    setMessage("+ Novas peças! Continue rápido!");
  }
  ...
  function endGame() {
    ...
    if (score > bestScore) {
      ...
      if (bestEl) bestEl.textContent = bestScore;
      setMessage(`Tempo esgotado! NOVO RECORDE: ${score} pontos (${pairs} pares)! 🎉`);
    } else {
      setMessage(`Tempo esgotado! ${score} pontos (${pairs} pares). Tente bater seu recorde de ${bestScore}!`);
    }
  ...
  function doHint() {
    ...
    const pair = findHintPair();
    if (!pair) {
      setMessage("Nenhuma dica — aguarde novas peças!");
      return;
    }
