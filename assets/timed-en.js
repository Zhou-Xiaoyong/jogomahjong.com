/*
 * Mahjong Timed — Solitaire against the clock.
 * Clear the board before the timer runs out. Traditional Chinese
 * tiles with annotations in English.
...
(function () {
  ...
  // Layout slightly smaller than the classic one to fit in the time.
  const layout = [
    ...rect(4, 0, 4, 1, 0),
    ...rect(2, 1, 8, 1, 0),
    ...rect(0, 2, 12, 4, 0),
  ...
  function startGame() {
    ...
    render();
    updateStats();
    setMessage("Race against time! Match identical free tiles before the clock runs out.");
  ...
  function endGame(won) {
    ...
    if (won) {
      ...
      setMessage(` Victory! You cleared the board with ${timeLeft}s left. Well done!`);
    } else {
      window.MAHJONG_SOUND?.error();
      setMessage(`Time's up! You removed ${removedPairs} of ${Math.floor(layout.length / 2)} pairs. Try again!`);
  ...
  function selectTile(uid) {
    ...
    if (!selected) {
      ...
      setMessage("Quick! Find the matching tile that is also free.");
      return;
    }
    ...
    if (matches(selected.type, tile.type)) {
      ...
      setTimeout(() => {
        ...
        if (tiles.every(item => item.removed)) {
          ...
          setMessage("Pair removed! +2 seconds. Keep going fast!");
        }
      }, 320);
    } else {
      ...
      markSelected();
      updateStats();
      setMessage("Tiles don't match. -1 second. Try another!");
    }
  }
  ...
  function showHint() {
    ...
    if (!pair) {
      setMessage("No free pairs. Use shuffle to continue (-3s for the hint).");
      return;
    }
    ...
    setMessage("Hint used: -3 seconds. Match the highlighted tiles.");
  }
  ...
  function shuffleRemaining() {
    ...
    render();
    updateStats();
    setMessage("Tiles shuffled: -5 seconds. Get to work!");
  }
